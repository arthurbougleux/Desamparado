...
resp = run(["time", os.getcwd()+"/./"+solver.strip(), out], text=True, stdout=PIPE).stdout
...
for v in resp:
    ...
    if v >= 0:
        ...
        ciclo.append([v//n +1, v%n +1])

ciclo.sort(key= lambda l: l[0])
for p in ciclo:
    #print(p[1], end=" ")
#print()
...
for i in range(1, len(ciclo)):
    ...
    if not g[ciclo[i-1][1]-1][ciclo[i][1]-1]:
...
if faz_sentido:
    ...
    #print("Vai aprender a programar")'''
