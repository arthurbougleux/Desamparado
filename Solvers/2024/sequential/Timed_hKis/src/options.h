...
#define OPTIONS \
  ...
  OPTION (bumpreasonslimit, 10, 1, INT_MAX, \
  ...
  OPTION( chrono, 0, 0, 1, "allow chronological backtracking") \
  ...
  OPTION (defraglim, 75, 50, 100, \
          "usable defragmentation limit in percent") \
  ...
  OPTION (eliminateclslim, 100, 1, INT_MAX, \
  ...
  OPTION (eliminateocclim, 2e3, 0, INT_MAX, \
  ...
  OPTION( psids, 0, 0, 1, "use PSIDS heuristics for deciding phases") \
  ...
  OPTION (reluctantlim, 1 << 20, 0, 1 << 30, \
          "reluctant limit (0=unlimited)") \
  ...
  OPTION (sweepmaxclauses, 32768, 2, INT_MAX, \
  ...
  OPTION (target, TARGET_DEFAULT, 0, 2, \
          "target phases (1=stable,2=focused)") \
  ...
  OPTION (vivifyirr, 1, 1, 100, "relative tier1 effort") \
  OPTION (vivifytier1, 1, 1, 100, "relative tier1 effort") \
